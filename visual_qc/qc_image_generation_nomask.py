...
percent_100 = np.percentile(img, 99)
percent_10 = np.percentile(img, 10)
...
slc = img.shape[0]/2 - 20
for i in range(3):
    fig.add_subplot(3, 3, 1+i)
    plt.imshow(np.rot90(img[slc+i*20, :, :]), cmap=cm.gray, vmin=percent_10,
               vmax=percent_100)
    #plt.imshow(np.rot90(rgbmsk[slc+i*20, :, :]), alpha=0.25)
    plt.axis('off')

slc = img.shape[1]/2 - 20
for i in range(3):
...
slc = img.shape[2]/2 - 20
for i in range(3):
    fig.add_subplot(3, 3, 7+i)
    plt.imshow(np.rot90(img[:, :, slc+i*20]), cmap=cm.gray, vmin=percent_10,
               vmax=percent_100)
    #mskplt.imshow(np.rot90(rgbmsk[:, :, slc+i*20]), alpha=0.25)
    plt.axis('off')
